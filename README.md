...
1.  **Vstup uživatele:** Uživatel poskytl PromptAlchymistovi detailní prompt šablonu pro AI asistenta v roli učitele/mentora. Jednalo se o šablonu 'AI Teacher Template', původně vytvořenou v rámci mého archivovaného projektu [AI Templates](https://github.com/painter99/ai-templates). Původní verzi této šablony v Markdownu si můžete prohlédnout [zde](https://github.com/painter99/ai-templates/blob/main/example/example-teaching-template-complete.md). Následně uživatel požádal o její refaktoring a optimalizaci pro model Gemini 2.5 Flash s cílem efektivně využít specifické schopnosti modelu, jako je **Controllable Reasoning ("thinking budget")** a **neustálé ověřování informací pomocí web search a grounding**.
